...
#error unsupport object format for HOST_I386
	...
	for (i = 0, rel = relocs, local16 = 0; i < nb_relocs; i++, rel++) {
		...
		switch(type) {
			case X86_64_RELOC_SIGNED_1:		// Signed displacement with a -1
				adjustment = -1;
				type = X86_64_RELOC_SIGNED;
				break;
			case X86_64_RELOC_SIGNED_2:		// Signed displacement with a -2
				adjustment = -2;
				type = X86_64_RELOC_SIGNED;
				break;
			case X86_64_RELOC_SIGNED_4:		// Signed displacement with a -4
				adjustment = -4;
				type = X86_64_RELOC_SIGNED;
				break;
		}
